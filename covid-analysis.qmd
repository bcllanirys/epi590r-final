...
regressionmodel <- tbl_uvregression(
  ...
  y=age, 
  method = lm,
  include = c(age, clinic_name, gender, payor_group, demo_group, patient_class)
)
regressionmodel

inline_text(model, variable= "patient_class")
...
model<- tbl_uvregression(
  ...
  y = age,
  method = lm,
  include = c(result, age, pan_day,clinic_name, payor_group, demo_group, patient_class),
  label = list(
    age ~ "Age",
    pan_day ~ "# of Days Tested After Pandemic Start",
    clinic_name ~ "Clinic",
...
#create function
inline_text(model, variable="pan_day", column="stat_1")
```
